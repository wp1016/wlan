<template>
 <div>
    ...
    <Table :columns="tableColumn" :data="tableData">
      ...
      <template slot="smsSmppServerIp" slot-scope="{index}">
        <Input v-model="list[index].smsSmppServerIp"></Input>
      </template>
      <template slot="smsSmppServerPort" slot-scope="{index}">
        <Input v-model="list[index].smsSmppServerPort"></Input>
      </template>
      ...
      tableColumn: [
        {
          ...
          render: (h, parmas) => {
            return h(CheckallList, {
              props: {
                value: this.list[parmas.index].smppClientPort
              },
              on: {
                change: ports => {
                  this.list[parmas.index].smppClientPort = ports
                }
              }
            })
          }
        ...
        },
        {
          key: 'smsSmppServerIp',
          title: '服务器地址',
          slot: 'smsSmppServerIp'
        },
        {
          key: 'smsSmppServerPort',
          title: '服务器端口',
          slot: 'smsSmppServerPort'
        },
        {
          key: 'smsSmppRecvCode',
          title: '收量编码',
